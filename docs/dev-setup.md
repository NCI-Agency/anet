...
1. Seed the initial data:
    1. If you're using the Docker container for the database (and you should), you can load the data with: `./gradlew dbLoad`. Otherwise, you'll need to manually connect to the database instance and load the data. Note that docker user must have read access to [insertBaseData.sql](../examples/simpleContainerDemo/insertBaseData.sql). This likely to fail in you home directory.
...
The following instructions initialize a database for testing purposes, withing the database container.
