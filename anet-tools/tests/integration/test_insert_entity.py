...
class InsertEntityTest(base_test_fixture.BaseTestFixture):
    ...
    def test_insert_entity_person(self):
        ...
        # generate result
        pp_from_query = self.session.query(self.PeoplePositions).filter(self.PeoplePositions.personUuid == self.new_uuid).all()[0]
    ...
    def test_insert_entity_pos_loc_org(self):
        ...
        # generate result
        location_from_query = self.session.query(self.Location).filter(self.Location.uuid == self.new_uuid).all()[0]
