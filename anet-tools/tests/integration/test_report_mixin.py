...
class TestReportMixin(base_test_fixture.BaseTestFixture):
    def test_associate_with_new_report(self):
        ...
        rp = (
            self.session.query(self.ReportPeople)
            .filter(self.ReportPeople.personUuid == person_1.uuid, self.ReportPeople.reportUuid == report.uuid)
            .all()
        )
        ...
        rp = (
            self.session.query(self.ReportPeople)
            .filter(self.ReportPeople.personUuid == person_2.uuid, self.ReportPeople.reportUuid == report.uuid)
            .all()
        )
        ...
        location_uuid = (
            self.session.query(self.Report).filter(self.Report.intent == report.intent).all()[0].locationUuid
        )
    ...
    def test_associate_with_existing_report(self):
        ...
        person_1 = self.Person(name="new_person", emailAddress="new_person@example.com", role=0)
        person_2 = self.Person(
            name="TOPFERNESS, Christopf", emailAddress="christopf_new_email_address@example.com", role=0
        )
        ...
        organization_uuid = (
            self.session.query(self.Report).filter(self.Report.intent == report.intent).all()[0].advisorOrganizationUuid
        )
