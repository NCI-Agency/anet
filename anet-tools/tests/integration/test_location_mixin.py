...
class TestLocationMixin(base_test_fixture.BaseTestFixture):
    def test_insert_location(self):
        ...
        location_uuid = self.session.query(self.Location).filter(self.Location.name == location.name).all()[0].uuid
    ...
    def test_update_location(self):
        ...
        location_status = self.session.query(self.Location).filter(self.Location.name == location.name).all()[0].status
        # generate result
