...
class base_methods:
    ...
    @staticmethod
    def relation_process(entity, relation_name, relation, entity_c, relation_c, update_rules, PeoplePositions, utc_now):
        if base_methods.is_entity_update(relation, update_rules):
            if relation_name == "person":
                base_methods.remove_persons_association_with_position(entity, PeoplePositions, utc_now)
                entity_c.currentPersonUuid = relation.uuid
            elif relation_name == "location":
                entity_c.locationUuid = relation.uuid
            elif relation_name == "organization":
                entity_c.organizationUuid = relation.uuid                
            relation.update_entity(utc_now)
            relation_c = None
        else:
            relation_c.uuid = relation.uuid
            relation_c.createdAt = utc_now
            relation_c.updatedAt = utc_now
    ...
    def remove_persons_association_with_position(position, PeoplePositions, utc_now):
        ...
        for pp in pp_list:
            if pp.endedAt is None:
                ...
                PeoplePositions.create(createdAt = utc_now, position = pp.positionUuid)
                pp.session.flush()
                break
        
    @staticmethod
    def remove_positions_association_with_person(position, PeoplePositions, utc_now):
        if position.currentPersonUuid == position.person.uuid:
            return
        
        pp_list = position.session.query(PeoplePositions).filter(and_(PeoplePositions.positionUuid == position.uuid, PeoplePositions.personUuid == position.currentPersonUuid)).all()

        if len(pp_list) == 0:
            return
        
        for pp in pp_list:
            if pp.endedAt is None and pp.personUuid is not None and pp.personUuid != position.person.uuid:
                pp.endedAt = utc_now
                pp.position.currentPersonUuid = None
