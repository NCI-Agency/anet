...
class anet_import(db):
    ...
    def write_data(self, entity_list_json):
        ...
        for entity_json in entity_list_json:
            ...
            counter = counter + 1
            if not self.verbose:
                print(str(counter) + "/" + str(len(entity_list_json)) + " passed", end="\r")
        print("Import is complete for entities whose table name is", entity_json["entity"].__tablename__)
    ...
    def write_unsuccessful_records_to_csv(self):
        if len(self.unsuccessful_entity_list_json) == 0:
            ...
            print("importing " + str(len(self.unsuccessful_entity_list_json)) + " entities unsuccessful. It is written to log file named: " + filename)
            print("\n")
    ...
    def exclude_old_entity_compare_hashes(self, entity_list_json):
        ...
        for e in entity_list_json:
            ...
            if str(hash(e["entity"])) not in self.hash_list:
    ...
    def write_successful_entities_hashfile(self):
        ...
        for entity_json in self.successful_entity_list_json:
            ...
            self.hash_list.append(str(hash(entity_json["entity"])))
        fullpath_hashfile = os.path.join(self.path_hashfile, "hashvalues.txt")
    ...
    def save_data(self, entity_list_json, verbose = True, remember_with_hash = True, write_unsuccessful=True):
        ...
        entity_list_json_incorrect_excluded = self.exclude_incorrect_entity(entity_list_json)
        if remember_with_hash:
            entity_list_json_old_excluded = self.exclude_old_entity_compare_hashes(entity_list_json_incorrect_excluded)
