...
class InsertUpdateNestedEntityTest(base_test_fixture.BaseTestFixture):
    ...
    def test_insert_update_nested_entity_people_update_pos_update_case1(self):
        ...
        pp_q = self.session.query(self.PeoplePositions).filter(self.PeoplePositions.personUuid == person.uuid, self.PeoplePositions.positionUuid == position.uuid).all()
    ...
    def test_insert_update_nested_entity_people_update_pos_update_case2(self):
        ...
        pp_q = self.session.query(self.PeoplePositions).filter(self.PeoplePositions.personUuid == None, self.PeoplePositions.positionUuid == formerPosUuid, self.PeoplePositions.endedAt == None).all()
        result = len(pp_q)
        expected = 1
        self.assertEqual(result, expected)        

        pp_q = self.session.query(self.PeoplePositions).filter(self.PeoplePositions.personUuid == None, self.PeoplePositions.positionUuid == position.uuid, self.PeoplePositions.endedAt == None).all()
        result = len(pp_q)
        expected = 1
        self.assertEqual(result, expected) 
        
        pp_q = self.session.query(self.PeoplePositions).filter(self.PeoplePositions.personUuid == person.uuid, self.PeoplePositions.positionUuid == position.uuid, self.PeoplePositions.endedAt == None).all()
        result = len(pp_q)
        expected = 1
        self.assertEqual(result, expected)
    ...
    def test_insert_update_nested_entity_people_update_pos_update_case3(self):
        ...
        #formerPosUuid = self.session.query(self.Position).filter(self.Position.currentPersonUuid == person.uuid).all()[0].uuid
        formerPersonUuid = self.session.query(self.Position).filter(self.Position.uuid == position.uuid).all()[0].currentPersonUuid
        ...
        pp_q = self.session.query(self.PeoplePositions).filter(self.PeoplePositions.personUuid == formerPersonUuid, self.PeoplePositions.positionUuid == None, self.PeoplePositions.endedAt == None).all()
        result = len(pp_q)
        expected = 1
        self.assertEqual(result, expected)                
