...
describe("Show organization page", () => {
  describe("As an admin", () => {
    ...
    it("Should see sync matrix on the EF2.2 Show page", async () => {
      ...
      expect(eventSeriesText).to.include("NMI PDT active test event")
      expect(eventSeriesText).to.include("Inactive test event series")
      expect(eventSeriesText).to.include("Active test event")
      ...
      expect(eventSeriesPrevText).to.not.include("NMI PDT active test event")
      expect(eventSeriesPrevText).to.not.include("Inactive test event series")
      expect(eventSeriesPrevText).to.not.include("Active test event")
  ...
  describe("When on the show page of an organization with events", () => {
    ...
    it("We can go to the show page of event", async () => {
      await (await ShowOrganization.getEvent(3)).click()
      await expect(await browser.getUrl()).to.include(
