  ...
  render() {
    return (
      <Table striped condensed hover responsive id="attendeesTable">
        ...
        <tbody>
          {Person.map(
            this.props.attendees.filter(p => p.role === Person.ROLE.ADVISOR),
          ...
          {Person.map(
            this.props.attendees.filter(p => p.role === Person.ROLE.PRINCIPAL),
            person => this.renderAttendeeRow(person)
          )}
        </tbody>
      </Table>
      ...
      <tr key={person.uuid}>
        <td className="primary-attendee">
          <Checkbox
            ...
            onChange={() =>
              !this.props.disabled && this.setPrimaryAttendee(person)
            }
          />
        </td>
        <td>
