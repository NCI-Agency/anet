...
function HistoryConflictWatcher({
  history,
  parentEntityUuid,
  ...
  useEffect(() => {
    const conflicts = getHistoryConflicts(history, parentEntityUuid)
...
function EditHistory({
  ...
  const getInitialState = useCallback(() => {
    const withUuid = giveEachItemUuid(initialHistory)
                    ...
                    <Row>
                      <Form className="form-horizontal">
                        ...
                        {values.history.map((item, idx) => {
                          ...
                          const isCurrent = !item.endTime

                          return (
                            <div key={item.uuid} style={getStyle(isError)}>
                              <Fieldset
                                title={`${idx + 1}-) ${item.primary ? "✔️ " : ""}${item[historyEntityType].name}${!item.endTime ? ` (Current ${historyEntityType})` : ""}`}
                                action={
                                  !isCurrent && (
                                    <RemoveButton
                                      title="Remove Item"
                                ...
                                <div className="inner-container">
                                  ...
                                  {!isCurrent ? (
                                    <Field
                                      ...
                                      label={null}
                                      disabled={!values.history[idx].endTime}
...
function getHistoryConflicts(history: any[], parentEntityUuid?: string) {
  ...
  history.forEach((current, i) => {
    ...
    history.forEach((other, j) => {
      ...
      if (current.primary && other.primary) {
    ...
    if (current.position?.previousPeople && parentEntityUuid) {
      const occupied = current.position.previousPeople.some(pp => {
        if (pp.person.uuid === parentEntityUuid) return false
