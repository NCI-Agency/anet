...
export default class Model {
  ...
  getPeriodAssessments(assessmentKey, period) {
    return this.assessments
      ...
      .map(assessment => ({
        assessment,
        assessmentValues: utils.parseJsonSafe(assessment.text)
      }))
      .filter(
        obj =>
  ...
  getInstantAssessmentResults(
    ...
    return this.assessments
      ...
      .map(assessment => utils.parseJsonSafe(assessment.text))
      .filter(
        obj =>
  ...
  static populateAssessmentsCustomFields(entity) {
    entity?.assessments?.forEach(assessment => {
      assessment.customFields = utils.parseJsonSafe(assessment.text)
    })
  }
