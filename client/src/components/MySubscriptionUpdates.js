...
class BaseMySubscriptionUpdates extends Component {
	...
	render() {
		...
		return <Fieldset title="My Subscription Updates">
			{subscriptionUpdatesExist ?
				...
				<Table striped condensed hover responsive className="subscriptionUpdates_table">
					<thead>
						<tr>
							<th />
							<th>Subscription</th>
					...
					<tbody>
						{subscriptionUpdates.map((subscriptionUpdate) => {
							...
							let subscribedObjectType = pluralize.singular(subscription.subscribedObjectType)
							...
							if (subscription.subscribedObject) {
								...
								linkToSubscription = <LinkTo {...linkToProps}>[object was deleted]</LinkTo>
							}
							let updatedObjectType = pluralize.singular(subscriptionUpdate.updatedObjectType)
							...
							if (subscriptionUpdate.updatedObject) {
								...
								if (subscriptionUpdate.isNote) {
									linkToUpdatedObject = <span>Note on {linkToUpdatedObject}</span>
								}
							} else {
								const linkToProps = {
									componentClass: 'span',
								...
								linkToUpdatedObject = <LinkTo {...linkToProps}>[object was deleted]</LinkTo>
							}
	...
	fetchSubscriptionUpdates = () => {
		...
		const subscriptionUpdatesPart = new GQL.Part(/* GraphQL */`
			...
			.addVariable("subscriptionUpdatesQuery", "SubscriptionUpdateSearchQueryInput", subscriptionUpdatesQuery)
...
export default connect(null, mapDispatchToProps)(BaseMySubscriptionUpdates)
