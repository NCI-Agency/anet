...
<Meta title="Anet User Stories/Vetting & Screening" />
...
is hightlighted by **<span style={{color: "green"}}>Valid until [date]</span>** text if the
...
- If a new assessment is added before the current one expires; in this case **<span style={{color:
  "grey"}}>No longer valid</span>** status will be displayed.
- If the current date is later than `Expiration date`; in this case **<span style={{color:
  "grey"}}>Expired</span>** status will be displayed (note that the text color would be red if there
  are no recent assessments).
