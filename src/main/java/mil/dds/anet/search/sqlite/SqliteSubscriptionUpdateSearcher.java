...
public class SqliteSubscriptionUpdateSearcher extends AbstractSearcherBase
  ...
  public AnetBeanList<SubscriptionUpdate> runSearch(SubscriptionUpdateSearchQuery query,
    ...
    final StringBuilder sql = new StringBuilder(
        "/* getSubscriptionUpdatesForPosition */ SELECT * FROM \"subscriptionUpdates\" "
    ...
    final AnetBeanList<SubscriptionUpdate> result = new AnetBeanList<>(query.getPageNum(),
        query.getPageSize(), new ArrayList<SubscriptionUpdate>());

    final Query q = getDbHandle().createQuery(sql.toString()).bindMap(args)
        .bind("offset", query.getPageSize() * query.getPageNum())
        .bind("limit", query.getPageSize());
    final List<SubscriptionUpdate> list = q.map(new SubscriptionUpdateMapper()).list();
    result.setList(list);
