...
public class MssqlSubscriptionUpdateSearcher extends AbstractSearcherBase
  ...
  public AnetBeanList<SubscriptionUpdate> runSearch(SubscriptionUpdateSearchQuery query,
    ...
    final StringBuilder sql = new StringBuilder("/* getSubscriptionUpdatesForPosition */ SELECT *"
        + ", count(*) over() as totalCount" + " FROM \"subscriptionUpdates\" "
    ...
    return new AnetBeanList<SubscriptionUpdate>(sqlQuery, query.getPageNum(), query.getPageSize(),
        new SubscriptionUpdateMapper(), null);
  }
