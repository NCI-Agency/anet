...
public class MssqlPersonSearcher extends AbstractMssqlSearcherBase<Person, PersonSearchQuery>
  ...
  public AnetBeanList<Person> runSearch(PersonSearchQuery query) {
    ...
    selectClauses.add(PersonDao.PERSON_FIELDS);
    ...
    if (doSoundex) {
      selectClauses.add("EXP(SUM(LOG(1.0/(5-DIFFERENCE(name_token.value, search_token.value)))))"
          + " AS search_rank");
      fromClauses.add("CROSS APPLY STRING_SPLIT(people.name, ' ') AS name_token"
      ...
      selectClauses.add("ISNULL(c_people.rank, 0) + ISNULL(f_people.rank, 0)"
