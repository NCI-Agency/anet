...
public class ForeignKeyByDateBatcher<T extends AbstractAnetBean> {
  ...
  public ForeignKeyByDateBatcher(String sql, String paramName, RowMapper<T> objectMapper,
  ...
  public List<List<T>> getByForeignKeys(List<ImmutablePair<String, Instant>> foreignKeys) {
    ...
    return foreignKeys.stream().map(resultsMap::get).map(l -> (l == null) ? new ArrayList<T>() : l)
