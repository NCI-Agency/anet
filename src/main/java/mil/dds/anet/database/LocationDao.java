...
public class LocationDao extends AnetBaseDao<Location, LocationSearchQuery> {
  ...
  public Location insertInternal(Location l) {
    getDbHandle().createUpdate(
        "/* locationInsert */ INSERT INTO locations (uuid, name, status, lat, lng, \"createdAt\", \"updatedAt\", \"customFields\") "
            + "VALUES (:uuid, :name, :status, :lat, :lng, :createdAt, :updatedAt, :customFields)")
  ...
  public int updateInternal(Location l) {
    return getDbHandle().createUpdate("/* updateLocation */ UPDATE locations "
        + "SET name = :name, status = :status, lat = :lat, lng = :lng, \"updatedAt\" = :updatedAt, \"customFields\" = :customFields WHERE uuid = :uuid")
