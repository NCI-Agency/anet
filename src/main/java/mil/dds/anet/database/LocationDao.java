...
public class LocationDao extends AnetBaseDao<Location> {
	...
	public LocationDao(AnetObjectEngine engine) {
		super(engine, "Locations", "locations", "*", null);
		final String idBatcherSql = "/* batch.getLocationsByUuids */ SELECT * from locations where uuid IN ( <uuids> )";
		this.idBatcher = new IdBatcher<Location>(engine, idBatcherSql, "uuids", new LocationMapper());
	}
	...
	public int updateInternal(Location l) {
		return engine.getDbHandle().createUpdate("/* updateLocation */ UPDATE locations "
	...
	public AnetBeanList<Location> search(LocationSearchQuery query) {
		return engine.getSearcher()
				.getLocationSearcher().runSearch(query, engine.getDbHandle());
	}
