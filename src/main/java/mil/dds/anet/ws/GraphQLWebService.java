...
public class GraphQLWebService {
  ...
  public ResponseEntity<Map<String, Object>> graphqlPostJson(Authentication authentication,
      @RequestBody GraphQLRequest graphQLRequest) {
    if (graphQLRequest == null || authentication == null) {
      return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
    }

    return ResourceTransformers.jsonTransformer
        .apply(graphQLResource.graphql((Principal) authentication.getPrincipal(), graphQLRequest));
  }
}
