 ...
 * Warning: The server does not support the BCC field.
 * Warning: The server appears to append an extra empty line at the end of the response's content.
 */
public class EmailResponse {
    ...
    public EmailResponse(JSONObject responseData) {
        this.from = responseData.keySet().contains("from")
                ? new ToFromData(responseData.getJSONObject("from"))
                : null;
        this.to = responseData.keySet().contains("to")
                ? new ToFromData(responseData.getJSONObject("to"))
                : null;
        this.cc = responseData.keySet().contains("cc")
                ? new ToFromData(responseData.getJSONObject("cc"))
                : null;
        ...
        this.date = responseData.keySet().contains("date")
                ? Instant.parse(responseData.getString("date"))
                : null;
