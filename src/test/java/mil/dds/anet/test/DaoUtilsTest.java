...
public class DaoUtilsTest {
	...
	public void testBuildPagedGetAllSql() {
		...
		String sqliteSql = DaoUtils.buildPagedGetAllSql(DbType.SQLITE, "Fakery", fakeTableName, fakeFieldList, orderBy);
		assertThat(sqliteSql).as("Generated SQL for SQLite")
		...
		assertThat(mssqlSql).as("Generated SQL for MS SQL Server")
			.contains("/* getAllFakery */")
			.contains("SELECT " + fakeFieldList)
			.contains(" FROM " +  fakeTableName)
			.contains("over() AS \"totalCount\"")
	...
	public void testDbTypeFromTag() {
		...
		try {
			...
			assertThat(e)
				.isExactlyInstanceOf(IllegalArgumentException.class)
				.hasMessageContaining("nopenopenope")
				.hasMessageStartingWith("No database type")
				;
		}
	}
