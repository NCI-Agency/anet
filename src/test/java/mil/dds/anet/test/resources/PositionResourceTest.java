...
public class PositionResourceTest extends AbstractResourceTest {
	...
	public void positionTest()
		...
		AnetBeanList<Position> retT = httpQuery(String.format("/api/positions/%s/associated", created.getUuid()), jack).get(new GenericType<AnetBeanList<Position>>(){});
		...
		retT = httpQuery(String.format("/api/positions/%s/associated", created.getUuid()), jack).get(new GenericType<AnetBeanList<Position>>(){});
	...
	public void createPositionTest()
		...
		assertThat(returned.loadPerson(context).get().getUuid()).isEqualTo(newb.getUuid());
		...
		assertThat(returned.loadPerson(context).get()).isNotNull();
		assertThat(returned.loadPerson(context).get().getUuid()).isEqualTo(prin2.getUuid());
