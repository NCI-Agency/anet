...
public class ReportsResourceTest extends AbstractResourceTest {
	...
	public void testSensitiveInformationByAuthor()
		...
		String returnedUuid = graphQLHelper.createObject(elizabeth, "createReport", "report", "ReportInput",
				r, new GenericType<GraphQLResponse<Report>>() {});
		assertThat(returnedUuid).isNotNull();
		Report returned = graphQLHelper.getObjectById(elizabeth, "report", FIELDS + " reportSensitiveInformation { text }", returnedUuid, new GenericType<GraphQLResponse<Report>>() {});
		assertThat(returned.getUuid()).isNotNull();
		...
		final Report returned2 = graphQLHelper.getObjectById(elizabeth, "report", FIELDS + " reportSensitiveInformation { text }", returned.getUuid(), new GenericType<GraphQLResponse<Report>>() {});
		...
		final Person jack = getJackJackson();
		final Report returned3 = graphQLHelper.getObjectById(jack, "report", FIELDS + " reportSensitiveInformation { text }", returned.getUuid(), new GenericType<GraphQLResponse<Report>>() {});
